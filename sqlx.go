...
func (db *DB) Exec(query string, args ...interface{}) (sql.Result, error) {
	...
	if err == nil {
		//r.RowsAffected() > 0
		if rs, err := ConfigArray.Parse(query, args...); err != nil && err != IncreNotSupportedError {
			fmt.Printf("Db SQL增量处理失败：%v，SQL：%s \n", err, query)
		} else if rs != nil {
			rs.Enqueue(args...)
		}
	}
...
func (db *Tx) Exec(query string, args ...interface{}) (sql.Result, error) {
	r, err := db.Tx.Exec(query, args...)
	if err == nil {
		//r.RowsAffected() > 0
		if rs, err := ConfigArray.Parse(query, args...); err != nil && err != IncreNotSupportedError {
			fmt.Printf("Tx SQL增量处理失败：%v，SQL：%s \n", err, query)
		} else if rs != nil {
			rs.Enqueue(args...)
		}
	}
